...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function optimizeCursor() {
    ...
    function animateCursor() {
      ...
      if (isVisible) {
        // Smooth interpolation for the cursor dot with hardware acceleration
    ...
    // Optimized throttled mouse move handler with passive event
    let lastMove = 0;
    document.addEventListener('mousemove', (e) => {
      const now = Date.now();
      if (now - lastMove > 16) { // ~60fps throttle for better performance
        mouseX = e.clientX;
        mouseY = e.clientY;
        lastMove = now;
      }
  ...
  }  // Check if viewport is at the expected size for proper section navigation
  function checkViewportSize() {
    // Add layout-specific classes based on viewport size
  ...
  function initAllFixes() {
    ...
    const cursorCleanup = optimizeCursor();
    ...
    const resizeObserver = new ResizeObserver(entries => {
      ...
      // Refresh section navigation after resize
    ...
    return function cleanup() {
      ...
      document.removeEventListener('sectionChanged', window.updateSectionNavButtons);
    };
  }

  // Run initialization after a brief delay to ensure other scripts are loaded
  const fixesCleanup = setTimeout(() => initAllFixes(), 500);
});
